...
FIELDS = [
    ...
    ("opening_bottom_radius", "Opening bottom round (nm)", "",
     "Round the BOTTOM of the opening into a U. 0 = flat; ≈ half the Space = full semicircle."),
    ("top_vacuum", "Top vacuum (nm)", "20",
     "Empty headroom drawn ABOVE the topmost material (room to deposit on top). This sits above the stack and does not affect Opening depth."),
]
OP_LABELS = ["Deposit · conformal", "Deposit · planar", "Fill",
             "Etch · isotropic", "Etch · anisotropic", "Planarize"]
...
def _op_to_row(op):
    o = op.get("op")
    if o == "conformal_deposit":
        return ("Deposit · conformal", op.get("material", "oxide"), op.get("thickness", 0), 1.0)
    if o == "planar_deposit":
        return ("Deposit · planar", op.get("material", "oxide"), op.get("thickness", 0), 1.0)
    if o == "fill":
        return ("Fill", op.get("material", "tungsten"), op.get("overfill", 0), 1.0)
    if o == "etch":
        a = op.get("anisotropy", 1.0)
        mat = op.get("material") or "(any)"
        return (("Etch · isotropic" if a <= 0 else "Etch · anisotropic"), mat, op.get("depth", 0), a)
    if o == "planarize":
        return ("Planarize", "oxide", op.get("at_height", 0), 1.0)
...
def compose_preview(bmp_path, nm_per_px, box_w=760, box_h=460, origin=(0.0, 0.0), return_scale=False):
    ...
    box_w = max(ML + MR + 80, int(box_w))
    box_h = max(MT + MB + 80, int(box_h))
    data_w = box_w - ML - MR
    data_h = box_h - MT - MB
    # letterbox, preserve aspect, don't over-zoom
    fit = min(data_w / w, data_h / h, 6.0)
    pw2, ph2 = max(1, int(w * fit)), max(1, int(h * fit))
    prof = img.resize((pw2, ph2), Image.NEAREST)
    px = ML + (data_w - pw2) // 2
    py = MT + (data_h - ph2) // 2      # centre the profile in the fixed data area
    canvas = Image.new("RGB", (box_w, box_h), (11, 22, 38))
    canvas.paste(prof, (px, py))
    ov = Image.new("RGBA", canvas.size, (0, 0, 0, 0))
    d = ImageDraw.Draw(ov)
    font = ImageFont.load_default()
    ppn = fit / nm_per_px
    wnm, hnm = w * nm_per_px, h * nm_per_px
    sx, sy = _nice_step(wnm), _nice_step(hnm)
    x0, y0 = origin
    grid = (110, 144, 198, 70)
    tick = (110, 144, 198, 255)
    frame = (70, 96, 140, 255)
    ...
    while nx <= x0 + wnm + 1e-6:
        xx = px + (nx - x0) * ppn
        if ML <= xx <= ML + data_w:
            d.line([(xx, MT), (xx, MT + data_h)], fill=grid)
            d.text((xx - 5, MT + data_h + 5), f"{int(round(nx))}", fill=tick, font=font)
        nx += sx
    ny = math.ceil(y0 / sy) * sy
    while ny <= y0 + hnm + 1e-6:
        yy = py + ph2 - (ny - y0) * ppn
        if MT <= yy <= MT + data_h:
            d.line([(ML, yy), (ML + data_w, yy)], fill=grid)
            d.text((6, yy - 4), f"{int(round(ny))}", fill=tick, font=font)
        ny += sy
...
class Tooltip:
    ...
    def _enter(self, _=None):
        self._cancel()
        self._show_job = self.w.after(300, self._show)

    def _show(self):
        if self.tip or not self.t:
            return
        ...
        y = self.w.winfo_rooty() + 22
        self.tip = tk.Toplevel(self.w)
        self.tip.wm_overrideredirect(True)
        self.tip.wm_geometry(f"+{x}+{y}")
        tk.Label(self.tip, text=self.t, justify="left", wraplength=260,
                 bg=NAVY_700, fg=ON, font=("", 9), padx=8, pady=6, bd=0).pack()
        Tooltip._active = self.tip
        self._hide_job = self.w.after(6000, self._leave)
    ...
    def _leave(self, _=None):
        ...
        if self.tip:
            try:
                ...
                pass
            if Tooltip._active is self.tip:
                Tooltip._active = None
            self.tip = None
...
class MaterialLayerRow:
    def __init__(self, app, material, thickness, shape=None):
        ...
        self.frame = ctk.CTkFrame(app.matstack_container, fg_color=NAVY_900,
                                  corner_radius=8, border_width=1, border_color=NAVY_700)
        ...
        self.grip = ctk.CTkLabel(top, text="⠿", font=app.ub,
                                 text_color=MUT, cursor="fleur", width=16)
        ...
        self.badge = ctk.CTkFrame(top, fg_color=BLUE, corner_radius=999, width=20, height=20)
        self.badge.pack(side="left", padx=(2, 6), pady=7)
        self.badge.pack_propagate(False)
        self.badge_lbl = ctk.CTkLabel(self.badge, text="1", font=app.eb, text_color=ON)
        self.badge_lbl.pack(expand=True)
        self.sw = ctk.CTkFrame(top, width=16, height=16, corner_radius=3, fg_color=app.palette.hex(
            material), border_width=1, border_color=NAVY_700)
        ...
        self.sw.pack(side="left", padx=(0, 6))
        self.sw.pack_propagate(False)
        ...
        self.mat.set(material)
        self.mat.pack(side="left", padx=(0, 6), pady=7)
        # --- bottom line: thickness + shape button, which used to get pushed off-screen ---
        bot = ctk.CTkFrame(self.frame, fg_color="transparent")
        bot.pack(fill="x", padx=2, pady=(0, 4))
        ctk.CTkLabel(bot, text="", width=26, fg_color="transparent").pack(
            side="left")   # indent under the badge
        self.th = ctk.CTkEntry(bot, width=64, font=app.mono, fg_color=NAVY_800,
                               border_color=NAVY_700, text_color=ON)
        self.th.insert(0, str(thickness))
        self.th.bind("<KeyRelease>", app._schedule_render)
        self.th.pack(side="left", padx=(0, 2))
        ...
        self.shape_btn.pack(side="left", padx=(4, 2))
        self._refresh_shape_btn()
    ...
    def _on_mat(self, _v):
        self.sw.configure(fg_color=self.app.palette.hex(self.mat.get()))
        self.app._refresh_op_materials()
...
class OpRow:
    ...
    def __init__(self, host, op_label, material, num, aniso=1.0):
        self.host = host
        app = host.app
        self.frame = ctk.CTkFrame(host.container, fg_color=NAVY_900,
                                  corner_radius=8, border_width=1, border_color=NAVY_700)
        ...
        for sym, cmd in (("✕", lambda: host.remove(self)), ("↓", lambda: host.move(self, 1)), ("↑", lambda: host.move(self, -1))):
            ctk.CTkButton(top, text=sym, width=26, font=app.uf, fg_color="transparent", border_width=1,
                          border_color=NAVY_700, text_color=SOFT, hover_color=NAVY_700, command=cmd).pack(side="right", padx=1)
        self.badge = ctk.CTkFrame(top, fg_color=BLUE, corner_radius=999, width=20, height=20)
        self.badge.pack(side="left", padx=(8, 6), pady=8)
        self.badge.pack_propagate(False)
        self.badge_lbl = ctk.CTkLabel(self.badge, text="1", font=app.eb, text_color=ON)
        self.badge_lbl.pack(expand=True)
        info = ctk.CTkLabel(top, text="ⓘ", font=app.uf, text_color=BLUE_L, cursor="hand2")
        info.pack(side="left", padx=(0, 4))
        Tooltip(info, OP_INFO)
        ...
        self.optype.set(op_label)
        self.optype.pack(side="left", padx=4, pady=6)
        # --- bottom line: material + thickness/aniso, which used to get pushed off-screen ---
        bot = ctk.CTkFrame(self.frame, fg_color="transparent")
        bot.pack(fill="x", padx=2, pady=(0, 4))
        ctk.CTkLabel(bot, text="", width=30, fg_color="transparent").pack(
            side="left")   # indent under the badge
        ...
        self.mat.set(material)
        self.mat.pack(side="left", padx=2, pady=6)
        self.num = ctk.CTkEntry(bot, width=56, font=app.mono,
                                fg_color=NAVY_800, border_color=NAVY_700, text_color=ON)
        self.num.insert(0, str(num))
        self.num.bind("<KeyRelease>", app._schedule_render)
        self.num.pack(side="left", padx=(8, 2))
        self.num_unit = ctk.CTkLabel(bot, text="nm", font=app.eb, text_color=MUT)
        self.num_unit.pack(side="left")
        self.aniso_lbl = ctk.CTkLabel(bot, text="aniso", font=app.eb, text_color=MUT)
        self.aniso = ctk.CTkEntry(bot, width=46, font=app.mono,
                                  fg_color=NAVY_800, border_color=NAVY_700, text_color=ON)
        self.aniso.insert(0, str(aniso))
        self.aniso.bind("<KeyRelease>", app._schedule_render)
        self._botrow = bot
        self._sync()
    ...
    def _sync(self):
        lbl = self.optype.get()
        names = self.host.app._op_material_choices()
        if lbl == "Planarize":
            ...
            if lbl in ("Etch · isotropic", "Etch · anisotropic"):
                ...
                if self.mat.get() not in vals:
                    self.mat.set("(any)")
            else:                       # Deposit / Fill introduce NEW material -> whole palette
                vals = self.host.app.palette.names()
                self.mat.configure(state="normal", values=vals)
                if self.mat.get() not in vals and vals:
                    self.mat.set(vals[0])
        if lbl == "Etch · anisotropic":
            self.aniso_lbl.pack(side="left", padx=(6, 1))
            self.aniso.pack(side="left")
        else:
            self.aniso_lbl.pack_forget()
            self.aniso.pack_forget()

    def to_op(self):
        lbl = self.optype.get()
        mat = self.mat.get()
        try:
            n = float(self.num.get())
        except ValueError:
            n = 0.0
        tgt = None if mat == "(any)" else mat
        if lbl == "Deposit · conformal":
            return dict(op="conformal_deposit", material=mat, thickness=n)
        if lbl == "Deposit · planar":
            return dict(op="planar_deposit", material=mat, thickness=n)
        if lbl == "Fill":
            return dict(op="fill", material=mat, overfill=n)
        if lbl == "Etch · isotropic":
            return dict(op="etch", depth=n, anisotropy=0.0, material=tgt)
        if lbl == "Etch · anisotropic":
            try:
                a = float(self.aniso.get())
            except ValueError:
                a = 1.0
            return dict(op="etch", depth=n, anisotropy=a, material=tgt)
        if lbl == "Planarize":
            return dict(op="planarize", at_height=n)
        return dict(op="conformal_deposit", material=mat, thickness=n)
...
class RepeatRow:
    def __init__(self, host):
        self.host = host
        app = host.app
        self.frame = ctk.CTkFrame(host.container, fg_color=NAVY_800,
                                  corner_radius=8, border_width=1, border_color=GREEN_D)
        head = ctk.CTkFrame(self.frame, fg_color="transparent")
        head.pack(fill="x", padx=8, pady=(8, 2))
        self.badge = ctk.CTkFrame(head, fg_color=GREEN_D, corner_radius=999, width=20, height=20)
        self.badge.pack(side="left", padx=(2, 8))
        self.badge.pack_propagate(False)
        self.badge_lbl = ctk.CTkLabel(self.badge, text="1", font=app.eb, text_color=ON)
        self.badge_lbl.pack(expand=True)
        ctk.CTkLabel(head, text="⟳ Repeat ×", font=app.ub, text_color=GREEN).pack(side="left")
        self.times = ctk.CTkEntry(head, width=44, font=app.mono,
                                  fg_color=NAVY_900, border_color=NAVY_700, text_color=ON)
        self.times.insert(0, "10")
        self.times.bind("<KeyRelease>", app._schedule_render)
        self.times.pack(side="left", padx=6)
        for sym, cmd in (("✕", lambda: host.remove(self)), ("↓", lambda: host.move(self, 1)), ("↑", lambda: host.move(self, -1))):
            ctk.CTkButton(head, text=sym, width=24, font=app.uf, fg_color="transparent", border_width=1,
                          border_color=NAVY_700, text_color=SOFT, hover_color=NAVY_700, command=cmd).pack(side="right", padx=1)
        addbar = ctk.CTkFrame(self.frame, fg_color="transparent")
        addbar.pack(fill="x", padx=8, pady=(0, 2))
        ...
        self.subcont = ctk.CTkFrame(self.frame, fg_color="transparent")
        self.subcont.pack(fill="x", padx=(14, 6), pady=(2, 8))
        self.sub = OpList(app, self.subcont)

    def to_op(self):
        try:
            n = int(float(self.times.get()))
        except ValueError:
            n = 1
...
class OpList:
    ...
    def add_op(self, kind="deposit"):
        self.app._capture()
        lbl, mat, num = OP_PRESETS.get(kind, OP_PRESETS["deposit"])
        self.rows.append(OpRow(self, lbl, mat, num))
        self.relayout()
        self.app.render_preview()
    ...
    def relayout(self):
        for n, row in enumerate(self.rows, 1):
            ...
            row.badge_lbl.configure(text=str(n))
...
class ProfileStudio(ctk.CTk):
    def __init__(self, build=True):
        ...
        self._drag = None
        self._last_npp = 0.4
        self.smooth_level = ctk.StringVar(value="Off")
        self._zoom = 1.0
        self._cx = 0.5
        self._cy = 0.5
        self._pv = None
        self._pan = None   # zoom/pan view state
        self._tool = "move"
        self._measure = False
        self._prof_wh = None
        self._hint_job = None
        ...
        self.tf = ctk.CTkFont(family="Inter", size=20, weight="bold")
    ...
    def build(self):
        ...
        self.bind_class("Entry", "<FocusIn>", lambda e: self._capture()
                        )   # one undo step per field edit
        self.after(160, self.render_preview)

    def _row(self, parent, r, label, info, widget):
        ctk.CTkLabel(parent, text=label, font=self.uf, text_color=SOFT).grid(
            row=r, column=0, sticky="w", padx=(8, 6), pady=5)
        ic = ctk.CTkLabel(parent, text="ⓘ", font=self.uf, text_color=BLUE_L, cursor="hand2")
        ic.grid(row=r, column=1, sticky="w", padx=(0, 8))
        Tooltip(ic, info)
    ...
    def _header(self):
        ...
        vlbl = ctk.CTkLabel(pill, text=short_version(), font=self.mono, text_color=GREEN_INK)
        vlbl.pack(padx=12, pady=3)
    ...
    def _card(self, parent, title):
        c = ctk.CTkFrame(parent, fg_color=NAVY_800, corner_radius=12,
                         border_width=1, border_color=NAVY_700)
        ctk.CTkLabel(c, text=title, font=self.ub, text_color=ON).pack(
    ...
    def _mat_menu(self, parent, default, width=150):
        ...
        m.set(default)
        self.material_menus.append(m)
        return m

    def _toolbar(self):
        tb = ctk.CTkFrame(self, fg_color=NAVY_800, corner_radius=0, height=36)
        ...
        tbtn("Save", self._save_project,
             "Save the whole project — materials, base feature, process stack — to a .json file", accent=True)
        sep()
    ...
    def _on_preview_resize(self, e):
        ...
        if self._loading or getattr(self, "_full_img", None) is None:
            return
        wh = (e.width, e.height)
        if wh == getattr(self, "_last_pv_wh", None):
            return
        self._last_pv_wh = wh
    ...
    def _session_tmp(self):
        ...
        if d is None or not Path(d).exists():
            ...
            try:
                os.chmod(d, 0o700)          # owner-only (no-op on Windows, which uses ACLs)
            except OSError:
                pass
            self._tmpdir = d
    ...
    def _fit_to_screen(self):
        ...
        try:
            ...
            try:
                wsf = ctk.ScalingTracker.get_window_scaling(self)
            except Exception:
                wsf = 1.0
            max_w = int(sw / wsf) - 40
            max_h = int(sh / wsf) - 90
    ...
    def _body(self):
        ...
        body.grid(row=2, column=0, sticky="nsew", padx=18, pady=16)
    ...
    def _inputs(self, parent):
        ...
        matf = ctk.CTkFrame(self.param_page, fg_color="transparent")
        matf.pack(fill="x", pady=(0, 6))
        ctk.CTkLabel(matf, text="MATERIAL STACK  (row ① = top)",
                     font=self.eb, text_color=BLUE_L).pack(anchor="w", padx=6)
        ...
        self.matstack_container = ctk.CTkFrame(matf, fg_color="transparent")
        self.matstack_container.pack(fill="x")
        self.matstack_hint = ctk.CTkLabel(
            matf, text="No layers yet — add one to start the stack.", font=ctk.CTkFont(size=11), text_color=MUT)
        self.matstack_hint.pack(anchor="w", padx=6)
        addrow = ctk.CTkFrame(matf, fg_color="transparent")
        addrow.pack(fill="x", padx=4, pady=(4, 0))
        ...
        csvf = ctk.CTkFrame(self.param_page, fg_color="transparent")
        csvf.pack(fill="x", pady=(8, 2))
        csvf.grid_columnconfigure(2, weight=1)
        ctk.CTkLabel(csvf, text="OPENING FROM CSV  (optional — defines the profile, overrides Space)",
                     font=self.eb, text_color=BLUE_L).grid(row=0, column=0, columnspan=3, sticky="w", padx=6, pady=(2, 2))
        ocsv = ctk.CTkFrame(csvf, fg_color="transparent")
        ocsv.grid(row=1, column=0, columnspan=3, sticky="ew", padx=6)
        ...
        self.csv_ref_entry = ctk.CTkEntry(csvf, font=self.mono, fg_color=NAVY_900,
                                          border_color=NAVY_700, text_color=ON, width=120, placeholder_text="top")
        ...
        bs = ctk.CTkFrame(self.param_page, fg_color="transparent")
        bs.pack(fill="x", pady=(8, 4))
        bs.grid_columnconfigure(2, weight=1)
        ctk.CTkLabel(bs, text="BASE FEATURE", font=self.eb, text_color=BLUE_L).grid(
            row=0, column=0, columnspan=3, sticky="w", padx=8, pady=(4, 2))
        for i, (k, label, default, info) in enumerate(FIELDS, 1):
            e = ctk.CTkEntry(bs, font=self.mono, fg_color=NAVY_900,
                             border_color=NAVY_700, text_color=ON, width=120)
            e.insert(0, default)
            e.bind("<KeyRelease>", self._schedule_render)
            self._row(bs, i, label, info, e)
            self.entries[k] = e

        ps = ctk.CTkFrame(self.param_page, fg_color="transparent")
        ps.pack(fill="x", pady=(8, 8))
        ctk.CTkLabel(ps, text="PROCESS STACK", font=self.eb,
                     text_color=BLUE_L).pack(anchor="w", padx=6)
        ...
        addbar = ctk.CTkFrame(ps, fg_color="transparent")
        addbar.pack(fill="x", padx=4)
        for text, kind in [("＋ Deposit", "deposit"), ("＋ Fill", "fill"), ("＋ Etch", "etch"), ("＋ Planarize", "planarize")]:
        ...
        self.op_container = ctk.CTkFrame(ps, fg_color="transparent")
        self.op_container.pack(fill="x", pady=4)
        self.empty_hint = ctk.CTkLabel(ps, text="No steps yet.",
                                       font=ctk.CTkFont(size=11), text_color=MUT)
        self.stack = OpList(self, self.op_container, empty_hint=self.empty_hint)
        self.empty_hint.pack(anchor="w", padx=6)

        common = ctk.CTkFrame(card, fg_color="transparent")
        common.pack(fill="x", padx=10, pady=(2, 8))
        common.grid_columnconfigure(2, weight=1)
        self.scale_entry = ctk.CTkEntry(common, font=self.mono, fg_color=NAVY_900,
                                        border_color=NAVY_700, text_color=ON, width=120, placeholder_text="auto")
        self.scale_entry.bind("<KeyRelease>", self._schedule_render)
        self._row(common, 0, "Resolution (nm/px)",
                  "Blank = auto-fit (~1000 px tall). Set a value to fix the nm-per-pixel calibration (smaller = higher resolution / bigger image).", self.scale_entry)

    def _preview_card(self, parent):
        card = self._card(parent, "Preview")
        card.grid(row=0, column=1, sticky="nsew")
        bottom = ctk.CTkFrame(card, fg_color="transparent")
        bottom.pack(side="bottom", fill="x")   # controls stay visible
        fr = ctk.CTkFrame(card, fg_color=NAVY_900, corner_radius=10)
        fr.pack(side="top", expand=True, fill="both", padx=18, pady=6)
        self.preview = ctk.CTkLabel(fr, text="", fg_color=NAVY_900)
        self.preview.pack(expand=True, fill="both", padx=10, pady=10)
        # inner widget that shows the image
        self._pv_target = getattr(self.preview, "_label", self.preview)
        self.preview.bind("<Configure>", self._on_preview_resize,
                          add="+")   # grow the plot with the window
        ...
        self.legend = ctk.CTkFrame(bottom, fg_color="transparent")
        ...
        sm = ctk.CTkFrame(bar, fg_color="transparent")
        sm.grid(row=0, column=0, sticky="w")
        ctk.CTkLabel(sm, text="Smoothing", font=self.eb,
                     text_color=SOFT).pack(side="left", padx=(0, 6))
        ...
        self.preview_note = ctk.CTkLabel(
            bar, text="preview updates as you edit", font=self.eb, text_color=MUT)
        self.preview_note.grid(row=0, column=1, padx=(0, 10))
        ...
        poly_btn.grid(row=0, column=2, padx=(0, 6))
        ...
        ctk.CTkButton(bar, text="Save .bmp…", command=self.save_bmp, font=self.ub, width=110,
                      fg_color=GREEN, hover_color=GREEN_D, text_color=GREEN_INK).grid(row=0, column=3)
        # zoom bar (row 1): drag the preview to pan, scroll to zoom, double-click to reset
        zb = ctk.CTkFrame(bar, fg_color="transparent")
        zb.grid(row=1, column=0, columnspan=3, sticky="ew", pady=(6, 2))
        zb.grid_columnconfigure(1, weight=1)
        ctk.CTkLabel(zb, text="Zoom", font=self.eb, text_color=SOFT).grid(
        ...
        Tooltip(self.measure_btn,
                "Measure: drag a line on the preview to read its length in nm. Works at any zoom.")
        ...
        self.move_btn.configure(fg_color=GREEN, text_color=GREEN_INK,
                                border_color=GREEN)   # Move active by default
    ...
    def _pan_start(self, e):
        if getattr(self, "_measure", False):
            return self._meas_start_cb(e)
        if self._zoom <= 1.0:
            self._flash_hint(
                "Zoom in to move — scroll over the preview or use the Zoom slider, then drag.")
            return
        self._pan = (e.x, e.y, self._cx, self._cy)
        try:
            self._pv_target.configure(cursor="fleur")
        except Exception:
            pass
    ...
    def _pan_move(self, e):
        if getattr(self, "_measure", False):
            return self._meas_move_cb(e)
        if not self._pan or self._zoom <= 1.0 or not self._pv:
            return
        x0, y0, cx0, cy0 = self._pan
        z = self._pv[2]
        # profile's on-screen extent
        pw2, ph2 = getattr(self, "_prof_wh", (self._pv[0], self._pv[1]))
    ...
    def _nudge_pan(self, dx, dy):
        ...
        f = self.focus_get()
        if isinstance(f, (_tk.Entry, _tk.Text)):
            return          # don't hijack text editing
        self._cx += dx * 0.12 / self._zoom
        self._cy += dy * 0.12 / self._zoom
        self.render_preview(view_only=True)
    ...
    def _select_tool(self, tool):
        ...
        self._update_cursor()
        if tool != "measure":
            self.render_preview()      # clear any measure line
    ...
    def _to_disp(self, e):
        ...
        try:
            ...
            pass
        x = (e.x - ox) / s
        y = (e.y - oy) / s                             # -> composed (unscaled) coords
        return (min(max(x, 0), pw), min(max(y, 0), ph))

    def _meas_start_cb(self, e):
        if self._pv:
            self._meas_start = self._snap(self._to_disp(e))

    def _meas_move_cb(self, e):
        if not self._meas_start or getattr(self, "_base_disp", None) is None or not self._pv:
            return
        p0 = self._meas_start
        p1 = self._snap(self._to_disp(e))
        im = self._base_disp.copy()
        d = ImageDraw.Draw(im)
        d.line([p0, p1], fill=(79, 208, 147), width=2)
        for p in (p0, p1):
            d.ellipse([p[0] - 3, p[1] - 3, p[0] + 3, p[1] + 3], fill=(79, 208, 147))
    ...
    def _build_snap_edges(self):
        ...
        if base is None:
            self._snap_edges = None
            return
        if getattr(self, "_snap_for", None) is base:
    ...
    def _snap(self, p):
        ...
        i = int(np.argmin(d2))
        if d2[i] > (18.0**2):
            return p                             # too far -> leave the cursor free
    ...
    def _add_matlayer(self, material="silicon", thickness=20, capture=True):
        if capture:
            self._capture()
        self.matlayer_rows.append(MaterialLayerRow(self, material, thickness))
    ...
    def _remove_matlayer(self, row):
        self._capture()
        row.frame.destroy()
        self.matlayer_rows.remove(row)
        self._relayout_matstack()
        self.render_preview()

    def _move_matlayer(self, row, delta):
        i = self.matlayer_rows.index(row)
        j = i + delta
        if 0 <= j < len(self.matlayer_rows):
            self._capture()
            self.matlayer_rows[i], self.matlayer_rows[j] = self.matlayer_rows[j], self.matlayer_rows[i]
            self._relayout_matstack()
            self.render_preview()
    ...
    def _refresh_op_materials(self):
        def walk(lst):
            for r in lst.rows:
                if isinstance(r, OpRow):
                    r._sync()
                elif isinstance(r, RepeatRow):
                    walk(r.sub)
    ...
    def _relayout_matstack(self):
        for n, row in enumerate(self.matlayer_rows, 1):
            row.frame.pack_forget()
            row.frame.pack(fill="x", padx=6, pady=3)
            row.badge_lbl.configure(text=str(n))
        self.matstack_hint.pack_forget()
        if not self.matlayer_rows:
            self.matstack_hint.pack(anchor="w", padx=6)
    ...
    def _drag_drop(self, row):
        ...
        target = len(self.matlayer_rows) - 1
        for i, r in enumerate(self.matlayer_rows):
            if y < r.frame.winfo_rooty() + r.frame.winfo_height() / 2:
                target = i
                break
        if self.matlayer_rows.index(self._drag) != target:
            self._capture()
            self.matlayer_rows.remove(self._drag)
            self.matlayer_rows.insert(target, self._drag)
            self._relayout_matstack()
            self.render_preview()
        self._drag = None
    ...
    def _load_ops(self, oplist, ops):
        ...
        for op in ops:
            if op.get("op") == "repeat":
                r = RepeatRow(oplist)
                oplist.rows.append(r)
                self._load_ops(r.sub, op.get("steps", []))
                r.times.delete(0, "end")
                r.times.insert(0, str(op.get("times", 1)))
            else:
                lbl, mat, num, aniso = _op_to_row(op)
                r = OpRow(oplist, lbl, mat, num, aniso)
                oplist.rows.append(r)
        oplist.relayout()
    ...
    def _load_state(self, snap):
        self._loading = True
        try:
            for r in list(self.matlayer_rows):
                r.frame.destroy()
            self.matlayer_rows = []
            for m in snap["materials"]:
                self.matlayer_rows.append(MaterialLayerRow(
                    self, m["material"], m["thickness"], m.get("shape")))
            self._relayout_matstack()
            ...
            self.scale_entry.insert(0, snap["scale"])
            self.csv_ref_entry.delete(0, "end")
            self.csv_ref_entry.insert(0, snap.get("csv_ref", ""))
    ...
    def _edit_shape(self, row):
        ...
        dlg.title("Top-corner shape")
        dlg.geometry("440x380")
        dlg.configure(fg_color=NAVY_800)
        dlg.transient(self)
        ctk.CTkLabel(dlg, text=f"Top-corner shape · {row.mat.get()}",
                     font=self.ub, text_color=ON).pack(anchor="w", padx=16, pady=(12, 2))
        ...
        cont = ctk.CTkScrollableFrame(dlg, fg_color="transparent", height=210)
            ...
            for rec in trows:
                ...
                def fv(e, d):
                    try:
                        ...
                        return d
                if k == "round":
                    out.append({"kind": "round", "r": fv(rec["p1"], 0)})
                elif k == "chamfer":
                    out.append({"kind": "chamfer", "s": fv(rec["p1"], 0)})
                elif k == "taper":
                    out.append({"kind": "taper", "angle": fv(rec["p1"], 80)})
                else:
                    out.append({"kind": "facet", "angle": fv(
                        rec["p1"], 45), "depth": fv(rec["p2"], 0)})
            return out

        def commit():
            row.shape = to_dicts()
            row._refresh_shape_btn()
            self.render_preview()

        def add_tr(t=None):
            t = t or {"kind": "round", "r": 10}
            fr = ctk.CTkFrame(cont, fg_color=NAVY_900, corner_radius=8)
            fr.pack(fill="x", pady=3)
            kind = ctk.CTkOptionMenu(fr, values=["round", "chamfer", "facet", "taper"], width=90, font=self.uf, fg_color=NAVY_800,
                                     button_color=BLUE, button_hover_color=BLUE_L, text_color=ON)
            kind.set(t.get("kind", "round"))
            kind.pack(side="left", padx=(8, 4), pady=7)
            ic = ctk.CTkLabel(fr, text="ⓘ", font=self.uf, text_color=BLUE_L, cursor="hand2")
            ...
            l1 = ctk.CTkLabel(fr, text="", font=self.eb, text_color=MUT, width=44)
            l1.pack(side="left")
            p1 = ctk.CTkEntry(fr, width=52, font=self.mono, fg_color=NAVY_800,
                              border_color=NAVY_700, text_color=ON)
            p1.pack(side="left", padx=2)
            l2 = ctk.CTkLabel(fr, text="", font=self.eb, text_color=MUT, width=44)
            p2 = ctk.CTkEntry(fr, width=52, font=self.mono, fg_color=NAVY_800,
                              border_color=NAVY_700, text_color=ON)
            ...
            def sync(_=None):
                k = kind.get()
                if k == "facet":
                    l1.configure(text="angle°")
                    l2.configure(text="depth")
                    l2.pack(side="left")
                    ...
                    p2.pack_forget()
                    l2.pack_forget()
                    if k == "taper" and not p1.get().strip():
                        p1.insert(0, "80")
            kind.configure(command=lambda _v: (sync(), commit()))
            p1.bind("<KeyRelease>", lambda e: commit())
            p2.bind("<KeyRelease>", lambda e: commit())
            ...
            if t.get("kind") == "facet":
                p1.insert(0, str(t.get("angle", 45)))
                p2.insert(0, str(t.get("depth", 10)))
            elif t.get("kind") == "chamfer":
                p1.insert(0, str(t.get("s", 0)))
            elif t.get("kind") == "taper":
                p1.insert(0, str(t.get("angle", 80)))
            else:
                p1.insert(0, str(t.get("r", 0)))
            trows.append(rec)
            sync()
        for t in row.shape:
            add_tr(t)
        if not row.shape:
            add_tr()          # start with one visible row so the pattern is obvious
        ...
        ctk.CTkButton(bar, text="Done", command=dlg.destroy, font=self.ub, width=90,
                      fg_color=GREEN, hover_color=GREEN_D, text_color=GREEN_INK).pack(side="left", padx=2)
    ...
    def _edit_material_color(self, name):
        ...
        if not name or name not in self.palette.names():
            return
        start = self.palette.rgb(name)
        dlg = ctk.CTkToplevel(self)
        dlg.title(f"Color — {name}")
        dlg.geometry("460x230")
        dlg.minsize(460, 230)
        dlg.configure(fg_color=NAVY_800)
        dlg.transient(self)
        dlg.grab_set()
        ctk.CTkLabel(dlg, text=f"Color of “{name}”", font=self.ub,
                     text_color=ON).pack(anchor="w", padx=16, pady=(14, 2))
        ...
        hex_e = ctk.CTkEntry(row, width=110, font=self.mono, fg_color=NAVY_900,
                             border_color=NAVY_700, text_color=ON)
        hex_e.pack(side="left")
        hex_e.insert(0, self.palette.hex(name))
        ...
        def _on_hex(_e=None):
            ...
            if rgb:
                _apply(rgb, echo=False)
            else:
                sw.configure(border_color="#E58B8B")
        hex_e.bind("<KeyRelease>", _on_hex)

        def pick():
            rgb, _ = askcolor(
                color=f"#{cur['rgb'][0]:02X}{cur['rgb'][1]:02X}{cur['rgb'][2]:02X}", parent=dlg, title=f"Color — {name}")
            if rgb:
                _apply(tuple(int(c) for c in rgb))
        ...
        def commit():
            ...
            self._apply_material_color(name, rgb)
            dlg.destroy()
    ...
    def _apply_material_color(self, name, rgb, already_set=False):
        ...
        for r in self.matlayer_rows:                       # refresh row swatches
            try:
                r.sw.configure(fg_color=self.palette.hex(r.mat.get()))
            except Exception:
                pass
    ...
    def _clear_error_tip(self, widget):
        tip = getattr(widget, "_err_tip", None)
        if tip is not None:
            tip.t = ""                    # nothing to reveal once it's healthy
    ...
    def _parse_hex(text):
        ...
        if len(s) == 3:
            s = "".join(c * 2 for c in s)          # #4d9 -> #44dd99
        if len(s) != 6:
            return None
        try:
    ...
    def _add_material(self):
        from tkinter.colorchooser import askcolor
        dlg = ctk.CTkToplevel(self)
        dlg.title("Add material")
        dlg.geometry("360x250")
        dlg.configure(fg_color=NAVY_800)
        dlg.transient(self)
        ctk.CTkLabel(dlg, text="New material", font=self.ub,
                     text_color=ON).pack(anchor="w", padx=16, pady=(14, 6))
        name_e = ctk.CTkEntry(dlg, placeholder_text="name (e.g. tungsten)",
                              font=self.uf, fg_color=NAVY_900, border_color=NAVY_700, text_color=ON)
        name_e.pack(fill="x", padx=16, pady=6)
        chosen = {"rgb": (79, 208, 147)}

        row = ctk.CTkFrame(dlg, fg_color="transparent")
        row.pack(fill="x", padx=16, pady=(4, 2))
        sw = ctk.CTkFrame(row, fg_color="#4FD093", width=40, height=28,
                          corner_radius=6, border_width=1, border_color=NAVY_700)
        sw.pack(side="left", padx=(0, 10))
        sw.pack_propagate(False)
        ctk.CTkLabel(row, text="Hex", font=self.eb, text_color=MUT).pack(side="left", padx=(0, 4))
        hex_e = ctk.CTkEntry(row, width=110, font=self.mono, fg_color=NAVY_900,
        ...
        def _on_hex(_e=None):
            rgb = self._parse_hex(hex_e.get())
            if rgb:
                _apply(rgb, echo_hex=False)          # valid: update swatch, keep the user's text
            else:
                sw.configure(border_color="#E58B8B")   # flag an invalid code without clobbering it
        hex_e.bind("<KeyRelease>", _on_hex)

        def pick():
            rgb, _ = askcolor(
                color=f"#{chosen['rgb'][0]:02X}{chosen['rgb'][1]:02X}{chosen['rgb'][2]:02X}", parent=dlg, title="Pick color")
            if rgb:
                _apply(tuple(int(c) for c in rgb))
        ctk.CTkButton(row, text="Pick…", command=pick, width=64, font=self.uf, fg_color="transparent",
                      border_width=1, border_color=BLUE_L, text_color=ON, hover_color=NAVY_700).pack(side="left", padx=(8, 0))
        ...
        def commit():
            ...
            # honour a typed hex even if not blurred
            rgb = self._parse_hex(hex_e.get()) or chosen["rgb"]
            if nm:
                ...
                self._update_menus()
            dlg.destroy()
        ctk.CTkButton(dlg, text="Add", command=commit, font=self.ub, width=90, fg_color=GREEN,
                      hover_color=GREEN_D, text_color=GREEN_INK).pack(side="right", padx=16, pady=14)
    ...
    def _load_csv(self):
        ...
        if not self.matlayer_rows and hspan > 0:      # give the opening something to cut, so it's visible
            self.matlayer_rows.append(MaterialLayerRow(self, "silicon", round(hspan, 1)))
            self._relayout_matstack()
        self._clear_error_tip(self.csv_label)
        self.csv_label.configure(
            text=f"✓ CSV loaded from {Path(path).name} ({len(df)} pts) — aligned to the TOP of the stack, cut downward. Set 'CSV height=0 at' to reposition; Space is ignored.", text_color=GREEN)
        self.render_preview()
    ...
    def _clear_csv(self):
        if self.opening_trace is not None:
            ...
            self.opening_trace = None
            self.csv_label.configure(
                text="Opening: rectangular (from Space). Load a CSV to use a trace profile instead.", text_color=MUT)
            self.render_preview()
    ...
    def _resolve_npp(self, st, cap=1600, target_px=1000):
        ...
        H = max(maxy - miny, 1.0)
        manual = self._scale()
        npp = manual if (manual and manual > 0) else H / \
            target_px  # manual value, else ~target_px px tall
        # clamp so neither side exceeds `cap` px
        npp = max(npp, W / cap, H / cap, 0.005)
        return npp

    def _render_to(self, out_path, for_save=False):
        base = proc.build_base(self._params())
        st = proc.evaluate(base, self.stack.to_ops())
        self._last_state = st
        z = max(1.0, getattr(self, "_zoom", 1.0))
        if for_save:
            npp = self._resolve_npp(st, cap=12000)
        else:
            npp = self._resolve_npp(st, cap=min(6000, int(1600 * z)),
                                    target_px=min(4000, int(1000 * z)))
        self._last_npp = npp
        ...
        while ss > 1 and maxdim * ss / npp > HI_CAP:
            ss //= 2
        # majority-vote AA, exact colors
        proc.render_regions(st, self.palette, out_path, npp, oversample=ss)
    ...
    def _reset(self, capture=True):
        ...
        for k, label, default, info in FIELDS:
            self.entries[k].delete(0, "end")
            self.entries[k].insert(0, default)
    ...
    def _update_legend(self):
        ...
        for mat in mats:
            ...
            sw = ctk.CTkFrame(chip, width=14, height=14, corner_radius=3,
                              fg_color=self.palette.hex(mat), border_width=1, border_color=NAVY_700)
            sw.pack(side="left", padx=(0, 5))
            sw.pack_propagate(False)
            lbl = ctk.CTkLabel(chip, text=mat, font=self.eb, text_color=SOFT)
            lbl.pack(side="left")
            for w in (sw, lbl):
    ...
    def _update_shape_availability(self):
        ...
        for r in rows:
            try:
                total += float(r.th.get())
            except ValueError:
                pass
        space = self._field("space")
        depth = self._field("opening_depth")
        trace = self.opening_trace is not None
        open_bottom = 0.0 if (depth is None or depth <= 0 or (
            total and depth >= total)) else (total - depth)
        y = 0.0
        topmap = {}
        for r in reversed(rows):
            ...
            topmap[id(r)] = y
        for r in rows:
            reach = (not trace) and (space is not None and space > 0) and (
                topmap.get(id(r), 0) > open_bottom + 1e-9)
    ...
    def render_preview(self, view_only=False):
        ...
        try:
            ...
            if view_only and cached is not None:
                ...
                self._render_to(tmp)
                im_full = Image.open(tmp).convert("RGB")
                im_full.load()
            ...
            try:
                _s = ctk.ScalingTracker.get_widget_scaling(self)
            except Exception:
                _s = 1.0
            box_w = _tgt.winfo_width() / _s
            box_h = _tgt.winfo_height() / _s
            if box_w < 120 or box_h < 120:
                box_w, box_h = 760, 460          # before first layout
            src_img = im_full
            origin = (0.0, 0.0)
            z = self._zoom
            if z > 1.0:
                ...
                data_w = max(1.0, box_w - ML - MR)
                data_h = max(1.0, box_h - MT - MB)
                ...
                hx = cw / (2 * W)
                hy = ch / (2 * H)                   # half-extents as fractions
                cx = min(max(self._cx, hx), 1 - hx)
                cy = min(max(self._cy, hy), 1 - hy)
                self._cx, self._cy = cx, cy
                fx0, fy0 = cx - hx, cy - hy
                px0 = int(fx0 * W)
                px1 = max(px0 + 1, int(round((cx + hx) * W)))
                py0 = int((1 - (cy + hy)) * H)
                py1 = max(py0 + 1, int(round((1 - fy0) * H)))  # image y is top-origin
                src_img = im_full.crop((px0, py0, px1, py1))
                origin = (fx0 * W * self._last_npp, fy0 * H * self._last_npp)
            disp, fit = compose_preview(src_img, self._last_npp, box_w=box_w,
                                        box_h=box_h, origin=origin, return_scale=True)
            # real nm per on-screen (composed) pixel
            self._nmpp_disp = self._last_npp / fit
            srcW, srcH = src_img.size
            self._prof_wh = (max(1, int(srcW * fit)), max(1, int(srcH * fit))
                             )   # profile's on-screen size (for panning)
            # clean image (for the measure overlay)
            self._base_disp = disp
            self._clear_error_tip(self.preview)
            self.preview.configure(image=ctk.CTkImage(
                light_image=disp, dark_image=disp, size=disp.size), text="")
            # for pan / measure mapping
            self._pv = (disp.size[0], disp.size[1], z)
            if not view_only:
                self._update_legend()   # materials can't change while panning
            if not view_only:
                ...
                self.preview_note.configure(
                    text=f"{self._last_npp:.3g} nm/px{mode} · updates live{tail}", text_color=MUT)
        except Exception as exc:
    ...
    def save_bmp(self):
        from tkinter import filedialog
        path = filedialog.asksaveasfilename(defaultextension=".bmp", filetypes=[
                                            ("Bitmap", "*.bmp")], initialfile="profile.bmp")
        if not path:
            return
    ...
    def export_polygons(self):
        ...
        if not path:
            return
        low = path.lower()
        stem = path[:-4] if (low.endswith(".svg") or low.endswith(".json")) else path
        try:
            base = proc.build_base(self._params())
            st = proc.evaluate(base, self.stack.to_ops())
            proc.export_polygons(st, self.palette, svg_path=stem +
                                 ".svg", json_path=stem + ".json")
        except Exception as exc:
