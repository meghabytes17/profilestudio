...
class LicenseGate(ctk.CTkToplevel):
    ...
    def _header(self):
        ...
        h.grid(row=0,column=0,sticky="ew"); h.grid_propagate(False); h.grid_columnconfigure(0,weight=1)
        ...
        ctk.CTkLabel(h,text=short_version(),font=self.mono,text_color=MUT).grid(row=0,column=1,sticky="e",padx=22)

    def _card(self,row,pady=(18,0)):
        c=ctk.CTkFrame(self,fg_color=NAVY_800,corner_radius=12,border_width=1,border_color=NAVY_700)
        c.grid(row=row,column=0,sticky="ew",padx=22,pady=pady); c.grid_columnconfigure(0,weight=1)
        return c
    ...
    def _activation_panel(self):
        self.panel=ctk.CTkFrame(self,fg_color=NAVY_800,corner_radius=12,border_width=1,border_color=NAVY_700)
        ...
        sig=ctk.CTkFrame(p,fg_color="transparent"); sig.grid(row=2,column=0,sticky="ew",padx=18)
        ...
        ctk.CTkFrame(p,height=1,fg_color=NAVY_700).grid(row=4,column=0,sticky="ew",padx=18,pady=(12,12))
        ...
        row=ctk.CTkFrame(p,fg_color="transparent"); row.grid(row=7,column=0,sticky="w",padx=18,pady=(0,16))
        ...
        self.pick_hint=ctk.CTkLabel(row,text="",font=self.eb,text_color=RED,justify="left",wraplength=330)
        self.pick_hint.pack(side="left",padx=(12,0))

    def _buttons(self):
        b=ctk.CTkFrame(self,fg_color="transparent"); b.grid(row=3,column=0,sticky="ew",padx=22,pady=(16,16))
        ...
        self.renew_btn=self._ghost(b,"Renew license…",lambda: self._toggle_panel(not self._panel_open),w=140)
        self.renew_btn.pack(side="right",padx=(0,8))
        self._ghost(b,"Quit",self._quit,w=80).pack(side="left")
        ctk.CTkLabel(b,text=version_string(),font=self.eb,text_color=MUT).pack(side="left",padx=(12,0))
    ...
    def _toggle_panel(self, show):
        ...
        if show: self.panel.grid(row=2,column=0,sticky="ew",padx=22,pady=(14,0))
        else: self.panel.grid_remove()
        self._resize()
    ...
    def _center(self):
        self.update_idletasks()
        w=self.winfo_width() or 600; h=self.winfo_height() or 460
        x=(self.winfo_screenwidth()-w)//2; y=max(0,(self.winfo_screenheight()-h)//3)
        self.geometry(f"+{x}+{y}")
    ...
    def _copy_signature(self):
        ...
        if not sig:
            self._flash(self.sig_hint,"No hardware signature available on this machine.",RED); return
    ...
    def _save_signature(self):
        ...
        try:
            ...
            self._flash(self.sig_hint,f"Could not write that file: {exc.strerror or exc}",RED); return
    ...
    def _pick_license(self):
        ...
        if not status.ok:
            self.pick_hint.configure(text=status.reason,text_color=RED); return
