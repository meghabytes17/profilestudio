...
const doc1=D([
  ...
  bullet([B("Round (radius)"),T(" — a fillet: a quarter-circle taken off the top corner.")]),
  bullet([B("Chamfer (size)"),T(" / "),B("Facet (angle + depth)"),T(" — straight-cut corners. Chamfer is a fixed "),B("45°"),T(" clip; facet does the same at any angle (depth = how far down the wall, angle measured from the horizontal top). A 45° facet is exactly a chamfer.")]),
  bullet([B("Taper (angle)"),T(" — slopes the "),B("whole"),T(" layer wall, not just the corner. Its angle is measured from the horizontal "),B("base"),T(": "),B("90° = vertical"),T(", smaller = more sloped.")]),
  bullet([T("A treatment larger than its layer is "),B("clamped to that layer"),T(", so it eases into the wall instead of leaving a step.")]),
  bullet([T("You can "),B("stack several treatments"),T(" on one layer (e.g. Round + Taper) via "),B("＋ Add another treatment"),T(" — they combine and are order-independent.")]),
  ...
  bullet([B("Move"),T(" — once zoomed in, drag (or use the arrow keys) to reposition. At 1× the whole profile is already in view, so there is nothing to move.")]),
  bullet([B("Measure"),T(" — drag a line across any feature to read it in nm. Set "),B("Snap"),T(" to Edge/Vertex to lock endpoints onto material boundaries. Switch back to "),B("Move"),T(" to pan.")]),
  bullet([B("Smoothing"),T(" (Off / 2× / 4× / 8×) de-jags curved walls without ever blending colours — the .bmp keeps exactly one colour per material.")]),
